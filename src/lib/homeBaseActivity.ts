...
export function initializePersonalizationStorage() {
  if (!isBrowser()) return;
  try {
    window.localStorage.removeItem(OLD_ACTIVITY_KEY);
  } catch {
    // Fail closed: old unclassified activity is never read or migrated.
  }
}

export function getPrefs(): PersonalizationPrefs {
  if (!isBrowser()) return DEFAULT_PREFS;
  try {
    const parsed = safeParse<Partial<PersonalizationPrefs>>(window.localStorage.getItem(PREFS_KEY), DEFAULT_PREFS);
    return { personalization: parsed.personalization !== false };
  } catch {
...
export function setPersonalization(enabled: boolean) {
  if (!isBrowser()) return;
  try {
    window.localStorage.setItem(PREFS_KEY, JSON.stringify({ personalization: enabled }));
    if (!enabled) {
...
export function suppressSession() {
  ...
  try {
    window.sessionStorage.setItem(SESSION_SUPPRESSION_KEY, '1');
  } catch {
    // A failed privacy write means callers should avoid recording anyway.
  }
}

export function isSessionSuppressed(): boolean {
  if (typeof window === 'undefined') return true;
  try {
    return Boolean(window.sessionStorage.getItem(SESSION_SUPPRESSION_KEY));
  } catch {
    return true;
  }
}
...
export function recordActivity(item: RecordableItem): void {
  ...
  try {
    ...
    if (!getPrefs().personalization) return;
    ...
    const next = [event, ...current.filter((entry) => entry.id !== item.id)].slice(0, 30);
...
function lookupPrivacy(id: string): ItemPrivacy {
  if (TOOLKIT_ITEM_BY_ID[id]) return TOOLKIT_ITEM_BY_ID[id].privacy ?? NEVER_PRIVACY;
  return ROUTES.find((route) => route.id === id)?.privacy ?? NEVER_PRIVACY;
}

export function readActivity(): ActivityEvent[] {
  if (!isBrowser()) return [];
  try {
    const events = safeParse<ActivityEvent[]>(window.localStorage.getItem(ACTIVITY_KEY), []);
    return events.filter((event) => lookupPrivacy(event.id).activity === 'allowed').slice(0, 30);
  } catch {
    return [];
  }
}
...
export function recordSupportVisit(pathname: string) {
  ...
  if (route) recordActivity(route);
}

export function recordToolkitItem(item: ToolkitItem) {
  recordActivity({
    id: item.id,
    title: item.title,
    section: item.section,
...
export function loadKitIds(): string[] {
  if (!isBrowser()) return [];
  try {
    const ids = safeParse<unknown[]>(window.localStorage.getItem(KIT_KEY), []);
    return ids.filter((id): id is string => typeof id === 'string' && Boolean(TOOLKIT_ITEM_BY_ID[id]));
  } catch {
    return [];
  }
}

export function toggleKitItem(id: string): string[] {
  if (!isBrowser() || !TOOLKIT_ITEM_BY_ID[id]) return loadKitIds();
  const current = loadKitIds();
  const next = current.includes(id) ? current.filter((item) => item !== id) : [...current, id].slice(-3);
  try {
    window.localStorage.setItem(KIT_KEY, JSON.stringify(next));
    emitChange();
  } catch {
    // Ignore storage failures.
  }
...
export function resetKit() {
  if (!isBrowser()) return;
  try {
    window.localStorage.removeItem(KIT_KEY);
    emitChange();
  } catch {
    // Ignore storage failures.
  }
...
export function removeAllCommonGroundDeviceData() {
  if (!isBrowser()) return;
  const keys = [ACTIVITY_KEY, KIT_KEY, PROGRESS_KEY, PREFS_KEY, 'cg-find-local-shortlist-v2', 'cg.carePlan.v1', 'cg.carePlanDraft.v1'];
  try {
...
export function loadHomeBaseActivity(): HomeBaseActivityState {
  ...
  return {
    version: 2,
    visits: events
      .filter((event) => event.kind !== 'worksheet')
      .map((event) => {
        ...
        return {
          ...
          href: event.href,
          label: event.title,
          description: route?.description ?? toolkitItem?.descriptor ?? 'A tool you opened recently.',
          kind: event.kind ?? (toolkitItem ? 'toolkit' : 'resources'),
          visitedAt: new Date(event.ts).toISOString(),
        };
      }),
    tools: events
      .filter((event) => event.kind === 'worksheet')
      .map((event) => ({ id: event.id, label: event.title, href: event.href, kind: 'worksheet' as const, openedAt: new Date(event.ts).toISOString() })),
    suggestionsEnabled: getPrefs().personalization,
  };
}

export function recordPrivateTool(tool: { id: string; label: string; href: string }) {
  recordActivity({
    id: `worksheet.${tool.id}`,
    title: tool.label,
    section: 'worksheet',
    href: tool.href,
    kind: 'worksheet',
    privacy: { activity: 'allowed', homeBase: 'allowed', sync: 'never' },
  });
}
