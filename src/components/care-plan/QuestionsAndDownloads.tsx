...
import { CARE_CLARITY_WORKSHEET } from '@/content/carePlanWorksheetLibrary';
...
export default function QuestionsAndDownloads({
  ...
  const usesCareClarityWorksheet = CARE_CLARITY_WORKSHEET.rows.has(row);
  const resolvedWorksheetHref = usesCareClarityWorksheet
    ? CARE_CLARITY_WORKSHEET.href
    : worksheetHref(worksheet.id);
  const resolvedWorksheetLabel = usesCareClarityWorksheet
    ? CARE_CLARITY_WORKSHEET.label.text
