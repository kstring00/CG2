...
/**
 * Persistent global crisis support bar.
 *
 * Project z-index convention (source of truth — keep this comment in sync):
 *   z-60  CrisisBar — global top bar, always on top (this component, via z-[60])
 *   z-50  ChatWidget FAB/panel, primary navs (homepage / client / privacy inline navs)
 ...
 * (defined in src/app/globals.css). globals.css adds top padding to <body>
 * equal to that variable so normal page content never sits under the bar,
 * and offsets known top-anchored chrome (fixed/sticky top-0 nav and headers)
 * by the same amount so they sit BELOW the bar.
 *
...
export default function CrisisBar() {
  ...
  return (
    <aside
      ...
      className="fixed inset-x-0 top-0 z-[60] border-b border-surface-border bg-surface-muted text-brand-muted-700"
    >
      ...
      {expanded && (
        <div
          id="crisis-bar-more"
          className="flex flex-col gap-1 border-t border-surface-border/70 bg-surface-muted px-3 py-2 text-[12px] sm:hidden"
        >
          <a
            href="tel:7139707000"
